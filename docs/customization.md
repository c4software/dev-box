...
`LANG` sets the locale of every shell of the box, SSH sessions and tmux panes
included: it wins over the `LANG` an SSH client sends. Two texts are
translated, in French only: the summaries of the `devbox` menu (and of
`devbox --help`) and the tips of the login line. Everything else, the output
of the commands, their `--help`, the docs and the skill, stays in English. Any
other language keeps the English text and only changes the formats (dates,
numbers, sorting).

The image has `C.UTF-8`, `en_US.UTF-8` and `fr_FR.UTF-8` ready. Any other
`xx_YY.UTF-8` is generated by the entrypoint the first time the container
starts with it (about a second). A value it cannot generate (a typo, a charset
other than UTF-8) is replaced by `C.UTF-8`, with a line in
`docker compose logs`, rather than leaving every program to print setlocale
warnings.

The English text is the source. The French one sits next to it: a
`# devbox:summary.fr=` line under the `# devbox:summary=` line of each
command, and `/usr/share/devbox/motd/tips.fr` for the tips, keyed by the
command of the `TIPS` list of `dev-box-motd`. A summary or a tip without a
translation is shown in English. The language code comes from `LC_ALL`, then
`LC_MESSAGES`, then `LANG` (`/usr/share/devbox/lib/lang.sh`).
...
- the `.env` settings the box can see from a shell (`TS_DISABLE`, `DEV_ENVS`,
  `LANG`, `LLM_PROXY_URL`, `PODMAN_ENABLE`, `DOTARCHY_*`, `USER_NAME`, `USER_SHELL`,
