...
| A change a user of the box notices | a line in the annotation of the next release tag (`git tag -a v1.7`): the workflow builds the amd64 image and turns it into the GitHub release that `devbox changelog` and the next login show. The arm64 image is built on demand afterwards: `git tag v1.7-arm v1.7 && git push origin v1.7-arm`, or `gh workflow run build-arm64.yml -f ref=v1.7` (or the Actions tab) |
